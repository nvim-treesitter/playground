...
local function lint_node(node, buf, error_type, complete_message)
  ...
  local node_text = table.concat(vim.treesitter.query.get_node_text(node, buf), " ")
...
function M.lint(query_buf)
  ...
  for _, m in pairs(matches) do
    ...
    if toplevel_node and query_lang then
      local query_text = table.concat(vim.treesitter.query.get_node_text(toplevel_node), "\n")
      local err
    ...
    if parser_info and parser_info.symbols then
      ...
      if node then
        local node_type = vim.treesitter.query.get_node_text(node)[1]
      ...
      if field_node then
        local field_name = vim.treesitter.query.get_node_text(field_node)[1]
